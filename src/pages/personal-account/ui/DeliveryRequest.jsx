...
const OrderCard = ({ order }) => {
  ...
  const cardStyles = order.status === 'wait' ? 'min-h-[100px] h-[100px]' : 'min-h-[120px] h-[150px]';

  const handleCardClick = () => {
    navigate(`/personal-account/delivery/order/${order.id}`);
  };
  ...
  return (
    <div onClick={handleCardClick} className={`relative border border-black rounded-sm bg-white gap-1 px-2 py-2 mb-4 flex flex-col min-w-[260px] w-[290px] ${cardStyles} shadow-sm hover:shadow transition-all cursor-pointer`}>
      ...
      {order.status === 'wait' && (
        <button className="bg-[#263554] text-white text-xs font-medium rounded px-2 py-1 mb-1 w-fit">Принять</button>
      )}
      {order.status === 'active' && (

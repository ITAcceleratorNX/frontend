...
const OrderCard = ({ order, isLoading = false, isDelivered = false }) => {
  ...
  const handleCardClick = () => {
    navigate(`/manager/moving/order/${order.movingOrderId}`);
  };
  ...
  return (
    <Card className="group cursor-pointer hover:shadow-lg transition-all duration-200 border-l-4 border-l-[#1e2c4f]">
      ...
      <CardContent className="space-y-4" onClick={handleCardClick}>
        ...
        {/* Кнопка действия - для менеджера всегда null */}
...
const ManagerMoving = () => {
  ...
  return (
    <div className="max-w-7xl mx-auto p-6 space-y-6">
      {/* Header */}
      <div className="space-y-2">
        <h1 className="text-3xl font-bold text-gray-900">Обзор заказов на перевозку</h1>
        <p className="text-gray-600">Мониторинг заказов на перевозку (только просмотр)</p>
      </div>
